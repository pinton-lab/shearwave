#!/usr/bin/env python3
"""Generate displacement snapshots + time traces figure from saved data.

Layout (3 rows × 3 cols):
  Row 0: Displacement XY snapshots at 3 time points
  Row 1: Displacement XZ snapshots at 3 time points
  Row 2: Time trace at peak ARF | lateral offsets | depth offsets
"""
...
# --- Paths ---
data_dir = Path("/home/gfp/fullwave25-private/outputs/sparse_transcranial_shear")
...
dt_frame = save_every * dt_shear  # time per saved frame
...
# --- ARF peak location ---
peak_idx = np.unravel_index(arf.argmax(), arf.shape)
ix0, iy0, iz0 = peak_idx
peak_depth_mm = ix0 * dx_ds_mm
peak_lat_mm = iy0 * dy_ds_mm
print(f"ARF peak at downsampled index ({ix0}, {iy0}, {iz0})")
print(f"  = ({peak_depth_mm:.1f}, {peak_lat_mm:.1f}) mm in (depth, lateral)")

# --- Pick 3 displacement time snapshots ---
# Choose frames corresponding to ~0.7 ms (early), ~1.7 ms (peak), ~3.0 ms (propagation)
...
lat_offsets_vox = [0, 5, 10, 15]  # voxels in downsampled grid
lat_labels = []
lat_traces = []
...
for i, dy_vox in enumerate(lat_offsets_vox):
    ...
    if 0 <= iy < ny_ds:
        ...
        xx = ix0 * dx_ds_mm
        ax_map.plot(
            yy, xx, "o", color=colors[i], markersize=6, markeredgecolor="white", markeredgewidth=0.8
        )
...
for i, dx_vox in enumerate(depth_offsets_vox):
    ...
    if 0 <= ixx < nx_ds:
        ...
        xx = ixx * dx_ds_mm
        ax_map.plot(
            yy, xx, "s", color=colors[i], markersize=6, markeredgecolor="white", markeredgewidth=0.8
        )
...
# --- Save ---
out_path = out_dir / "fig_de_displacement_and_traces.png"
