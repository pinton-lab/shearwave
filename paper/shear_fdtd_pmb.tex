...
($L_x=L_y=L_z=36$~mm), fixed physical sensor distances, and a six-level
refinement range. The Gaussian source width
...
are due purely to grid resolution. Table~\ref{tab:convergence} summarizes the recovered wavespeeds at each grid
resolution.  As the grid is refined from $n=60$ to $n=180$, the mean
speed $\bar{c}$ converges monotonically from 1.828 to 1.972~m/s and the
...
\caption{Grid convergence of recovered shear wavespeed (target $c_s = 2.000$~m/s,
  $L = 36$~mm fixed domain).  $\bar{c}$ and $c_{\mathrm{fit}}$ denote the
  mean and linear-fit wavespeeds, respectively.}
\centering
\begin{tabular}{c c c c}
\hline
$n$ & $\Delta x$ ($\mu$m) & $\bar{c}$ (m/s) & $c_{\mathrm{fit}}$ (m/s) \\
\hline
60  & 600 & 1.828 & 2.193 \\
80  & 450 & 1.907 & 2.048 \\
100 & 360 & 1.938 & 2.053 \\
...
error}, $|\bar{c}-c_{\mathrm{target}}|$, is the absolute difference
between the mean recovered speed and the analytical
...
difference at each sensor, averaged across sensors, measured against the
finest grid ($n=180$) as reference. It captures full-waveform convergence rather
than a single arrival time. Fitting $|\bar{c}-c_{\mathrm{target}}| \propto \Delta x^p$ in log--log space
gives a global slope of $p=1.68$ ($R^2=0.98$), and the trace $L_2$ metric gives a
...
\begin{figure}
\centering
\includegraphics[width=0.49\textwidth]{convergence_mean_error_only.png}%
\hfill
...
($|\bar{c} - c_{\mathrm{target}}|$) vs grid spacing (log--log).
