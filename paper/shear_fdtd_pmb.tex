...
\affil{$^1$Joint Department of Biomedical Engineering, University of North Carolina at Chapel Hill and North Carolina State University, Chapel Hill, NC, USA}
...
(trace $L_2$ slope 1.83), and agreement with analytical Kelvin--Voigt
...
generates transient elastic displacements whose propagation encodes the
underlying shear modulus of tissue. In ultrasonic neuromodulation, related
elastomechanical interactions have been proposed as potential mechanisms
linking acoustic exposure to neural
...
computationally efficient modeling of elastic wave generation and propagation in
soft solids under ultrasound excitation.

Extensive prior work has developed high-fidelity simulation frameworks for SWE
based on general-purpose finite element methods
(FEM)~\cite{hughes2000finite,zienkiewicz2005finite}, including widely used
packages such as LS-DYNA and
Abaqus~\cite{bercoff2004supersonic,capuzzi2018numerical}. These tools can
represent complex geometries, heterogeneous materials, nonlinear constitutive
behavior, and boundary interactions, making them highly versatile. However, this
generality comes at substantial computational and algorithmic cost. For
ultrasound-specific problems---where the primary interest lies in small,
radiation-force--induced shear displacements in nearly incompressible soft
tissue---the full generality of large-scale FEM is often unnecessary and can
obscure the essential physics.
...
solver~\cite{yang2018gpu}. Such sequential acoustic--elastic coupling
...
The objective of the present work is to derive and implement a reduced,
...
radiation-force--induced response of interest in
elastography~\cite{sandrin2003transient,tanter2008quantitative} is dominated by
slower shear motion governed primarily by $\mu$. This motivates (i)
shear-focused elastic models and (ii) sequential acousto--elastic workflows
where the acoustic field is used to compute forcing, and the subsequent shear
dynamics are simulated on a much slower time axis.
...
of magnitude faster than shear propagation, the force distribution may be
computed independently using an acoustic simulation and treated as a prescribed
source term in Eq.~\eqref{eq:shear_reduced_kv}.
...
or time-varying aberration correction.
...
motion; the solver is a shear-mode propagator, not a full elastic-wave solver.
...
Conjugate gradient (CG)~\cite{shewchuk1994conjugate,golub2013matrix} is a
...
Each CG solve costs $O(N\,n_{\mathrm{iter}})$ with $N$ grid points, giving a
total projection cost of $O(N \cdot n_{\mathrm{iter}} \cdot N_t)$ over $N_t$
time steps.
...
In the standard ARFI scenario the acoustic intensity field is computed once and
...
successive pushes at different foci), with only $O(N)$ scaling required during
...
The acoustic field can be obtained from any suitable solver---FDTD codes such
as Fullwave~\cite{pinton2020fullwave} or
k-Wave~\cite{treeby2010modeling}, pseudo-spectral methods, or the angular
spectrum approach~\cite{zeng2008evaluation}---and need not be computed with
the same tool as the shear solver.

\paragraph{Acoustic radiation force.}
Two formulations are used to compute the body force from the acoustic field.
...
frequency. For converging beams the plane-wave assumption breaks down near the
focal zone; a more accurate approach uses the time-averaged Poynting vector
...
When the acoustic simulation uses short pulsed excitations to reduce
computational cost, multiplicative corrections are applied to extrapolate the
computed force to the equivalent continuous-wave or longer-pulse regime.
...
compressional content, providing a clean test of the shear propagation numerics
without contamination from longitudinal waves or from the Helmholtz projection
step.

The simulation domain is a cubic $180^3$ grid ($L_x=L_y=L_z=36$~mm,
...
drives motion in the $x$--$z$ plane; the center $x$-plane is a nodal surface
for $u_z$ by symmetry. Wavespeed is estimated from peak arrival times in the
sensor traces.
...
recovered speed across the four sensors is 1.976~m/s (${\sim}$1.2\% below
...
($L_x=L_y=L_z=36$~mm), fixed physical sensor distances (first four sensors
only), and a six-level refinement range. The Gaussian source width
($\sigma_{xy}=5$ grid points) and velocity damping coefficient
($\gamma_v=0.001$) are held constant across all levels so that any differences
are due purely to grid resolution.

Table~\ref{tab:convergence} summarizes the recovered wavespeeds at each grid
resolution.  As the grid is refined from $n=60$ to $n=180$, the mean
four-sensor speed converges monotonically from 1.828 to 1.972~m/s and the
linear-fit speed converges from 2.193 to 1.998~m/s.

\begin{table}
\caption{Grid convergence of recovered shear wavespeed (target $c_s = 2.000$~m/s,
  $L = 36$~mm fixed domain).  mean$_4$ and $c_{\mathrm{fit},4}$ denote the
  four-sensor mean and linear-fit wavespeeds, respectively.}
\centering
\begin{tabular}{c c c c}
\hline
$n$ & $\Delta x$ ($\mu$m) & mean$_4$ (m/s) & $c_{\mathrm{fit},4}$ (m/s) \\
\hline
60  & 600 & 1.828 & 2.193 \\
80  & 450 & 1.907 & 2.048 \\
100 & 360 & 1.938 & 2.053 \\
...
Two complementary error metrics quantify convergence.  The \emph{wavespeed
error}, $|\mathrm{mean}_4-c_{\mathrm{target}}|$, is the absolute difference
between the mean recovered speed over the first four sensors and the analytical
target; it is a scalar metric derived from discrete peak-picking.  The
\emph{trace $L_2$ error} is the relative $L_2$ norm of the velocity trace
difference at each sensor, averaged over four sensors, measured against the
finest grid ($n=180$) as reference; it captures full-waveform convergence rather
than a single arrival time.

\begin{figure}
\centering
\includegraphics[width=0.49\textwidth]{convergence_mean4_error_only.png}%
\hfill
\includegraphics[width=0.49\textwidth]{convergence_trace_l2.png}
\caption{Grid convergence study. \textbf{Left:} Wavespeed convergence
($|\mathrm{mean}_4 - c_{\mathrm{target}}|$) vs grid spacing (log--log).
Fitted slope 1.68, $R^2=0.98$. \textbf{Right:} Trace $L_2$ convergence
(relative $L_2$ error of velocity traces vs finest grid). Fitted slope 1.83,
$R^2=0.90$. Dashed reference lines show $O(\Delta x)$ and
$O(\Delta x^2)$.}
\label{fig:convergence}
\end{figure}

Fitting $|\mathrm{mean}_4-c_{\mathrm{target}}| \propto dX^p$ in log--log space
gives a global slope of $p=1.68$ ($R^2=0.98$); the trace $L_2$ metric gives a
global slope of $p=1.83$ ($R^2=0.90$).
Both metrics confirm convergence consistent with the second-order spatial
discretization (7-point Laplacian stencil) and second-order leapfrog time
integration. The trace $L_2$ metric, which captures the full waveform rather
than a single scalar, yields a cleaner slope ($p\approx1.83$) closer to the
theoretical $O(\Delta x^2)$ rate. At the finest level ($n=180$), the
linear-fit speed reaches 1.998~m/s ($<$0.1\% error).
...
To align with Kelvin--Voigt theory and cover a brain-relevant range, the
...
attenuation fit low. Sensor positions are adapted per $\eta$ based on the
...
The combination of adaptive sensor placement (keyed to the theoretical
e-folding length) and dimensional-analysis-guided frequency scheduling yields
...
following the \texttt{GaussExc} convention used in the QIBA FEM
...
\centering
  ...
  excitation matching the QIBA \texttt{GaussExc} convention.
  ...
  $G=5$~kPa phantom; the dashed line indicates the analytical shear wave
...
section demonstrates the complete acoustic--elastic pipeline
(Section~\ref{sec:coupling}) applied to transcranial focused ultrasound through
a realistic human skull. Two transducer configurations are considered: a
1024-element sparse phased array and the Philips TIPS curved-bowl transducer.
Both simulations use the same \emph{ex~vivo} micro-CT skull specimen (Halle
temporal bone) and share the same four-stage workflow: (i)~acoustic transmit
...
750~$\mu$m element pitch. Element positions are loaded from an HDF5 connector
...
the Halle temporal bone specimen. Hounsfield units are mapped to spatially
varying sound speed (1540--2900~m/s), density (1000--2200~kg/m$^3$), and
attenuation coefficient. The background tissue is assigned standard soft-tissue
properties. The acoustic simulation uses the Fullwave GPU FDTD solver with
...
designed for transcranial focused ultrasound therapy and neuromodulation. The
...
The solver is derived under a small-strain linearization: displacement gradients
...
Table~\ref{tab:solver_comparison} compares the reduced FDTD formulation with
the principal existing approaches for simulating ARF-induced shear wave
propagation.

\begin{table}
\caption{Comparison of shear wave simulation approaches.}
\centering
\begin{tabular}{l c c c}
\hline
Feature & FEM & Green's function & This FDTD \\
\hline
Spatial heterogeneity & Yes & Limited & Yes \\
Arbitrary source geometry & Yes & Yes & Yes \\
Full-field output & Yes & No (point-wise) & Yes \\
Compressional waves & Yes & No & No \\
Typical setup cost & High & Low & Low \\
Computational cost & High & Low--moderate & Low \\
Code availability & Commercial & Custom & Open-source \\
\hline
\end{tabular}
\label{tab:solver_comparison}
\end{table}

FEM approaches~\cite{palmeri2017fem,palmeri2005fem} offer the greatest
...
Table~\ref{tab:arf_scaling} illustrates this effect at a fixed focal intensity
of $I_{\mathrm{SPPA}} = 1000$~W/cm$^2$.
...
\subsection{When valid and when not valid}
...
Beyond the transcranial demonstrations in Section~\ref{sec:transcranial_demos},
the supplementary material presents a theta-burst transcranial ultrasound
stimulation (TUS) protocol simulation and an analysis connecting tissue
displacement gradients to membrane tension and mechanosensitive channel gating
...
and $<$0.5\%, respectively). The pre-computed Helmholtz projection for separable
body forces reduces simulation time by one to two orders of magnitude in
typical ARF-driven scenarios.

The principal limitations are those implied by the model reduction: compressional
...
\funding{This work was supported in part by NIH grants R01-EB032801 and R01-NS130069.}
...
\data{The simulation code is available as part of the Fullwave 2.5 package at \url{https://github.com/pinton-lab/fullwave25}. Validation scripts are provided in the \texttt{fullwave/shear\_wave/validation/} directory.}

\suppdata{Supplementary material includes a theta-burst TUS protocol simulation and an analysis connecting tissue displacement to membrane tension and mechanosensitive channel gating for neuromodulation applications.}
